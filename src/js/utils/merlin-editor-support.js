...
// Width of the opcode field. A longer opcode — a macro name, say — pushes the
// operand along instead of being padded into it, so this is the nominal width
// rather than a limit.
export const OPCODE_WIDTH = COL_OPERAND - COL_OPCODE;
...
const DIRECTIVE_INFO = {
  ORG: { syntax: "ORG addr", desc: "Set the assembly address" },
  OBJ: { syntax: "OBJ addr", desc: "Object buffer address (accepted, no effect)" },
  EQU: { syntax: "label EQU value", desc: "Equate label to value" },
  "=": { syntax: "label = value", desc: "Equate label to value" },
  VAR: { syntax: "VAR v1;v2;...", desc: "Load the numbered variables ]1..]8" },
  DS: { syntax: "DS count[,fill] | DS \\", desc: "Reserve space; \\ pads to the next page" },
  DUM: { syntax: "DUM addr", desc: "Start a dummy section (addresses, no code)" },
  DEND: { syntax: "DEND", desc: "End a dummy section" },
  DFB: { syntax: "DFB byte[,byte]...", desc: "Define byte(s)" },
  DB: { syntax: "DB byte[,byte]...", desc: "Define byte(s), alias for DFB" },
  DW: { syntax: "DW word[,word]...", desc: "Define word(s), little-endian" },
  DA: { syntax: "DA addr[,addr]...", desc: "Define address(es), alias for DW" },
  DDB: { syntax: "DDB word[,word]...", desc: "Define double byte (big-endian)" },
  ADR: { syntax: "ADR addr[,addr]...", desc: "3-byte address" },
  ADRL: { syntax: "ADRL addr[,addr]...", desc: "4-byte address" },
  HEX: { syntax: "HEX bytes", desc: "Hex data (no $ prefix)" },
  CHK: { syntax: "CHK", desc: "Emit the checksum of the object so far" },
  ASC: { syntax: 'ASC "text"', desc: "String; \' is ASCII, \" sets the high bit" },
  DCI: { syntax: 'DCI "text"', desc: "String, last character's high bit flipped" },
  INV: { syntax: 'INV "text"', desc: "String in the inverse character set" },
  FLS: { syntax: 'FLS "text"', desc: "String in the flashing character set" },
  REV: { syntax: 'REV "text"', desc: "String stored back to front" },
  STR: { syntax: 'STR "text"', desc: "String with a leading length byte" },
  STRL: { syntax: 'STRL "text"', desc: "String with a leading 2-byte length" },
  DO: { syntax: "DO expr", desc: "Assemble the block if expr is non-zero" },
  IF: { syntax: 'IF "c",]1', desc: "Assemble if the first character matches" },
  ELSE: { syntax: "ELSE", desc: "Assemble the other branch" },
  FIN: { syntax: "FIN", desc: "End a DO or IF block" },
  LUP: { syntax: "LUP count", desc: "Repeat the block count times" },
  '--^': { syntax: "--^", desc: "End of a LUP block" },
  ELUP: { syntax: "ELUP", desc: "End of a LUP block (alt syntax)" },
  END: { syntax: "END", desc: "Stop assembling here" },
  ERR: { syntax: "ERR expr", desc: "Fail the assembly if expr is non-zero" },
  MAC: { syntax: "name MAC", desc: "Begin a macro definition" },
  EOM: { syntax: "EOM", desc: "End of macro" },
  '<<<': { syntax: "<<<", desc: "End of macro (alt syntax)" },
  '>>>': { syntax: ">>> name[;p1;p2]", desc: "Call a macro" },
  PMC: { syntax: "PMC name[;p1;p2]", desc: "Call a macro" },
  PUT: { syntax: "PUT filename", desc: "Include source from the disk in a drive" },
  USE: { syntax: "USE filename", desc: "Include source from the disk in a drive" },
  CHN: { syntax: "CHN filename", desc: "Continue assembling in another file" },
  DSK: { syntax: "DSK filename[,D1|D2]", desc: "Write object to the disk in a drive" },
  SAV: { syntax: "SAV filename[,D1|D2]", desc: "Write object to the disk in a drive" },
  TYP: { syntax: "TYP type", desc: "ProDOS file type for the object file" },
  LST: { syntax: "LST ON|OFF", desc: "Listing on or off" },
  CYC: { syntax: "CYC ON|OFF", desc: "Cycle counts in the listing" },
  SW: { syntax: "SW [OFF]", desc: "Sweet-16 mnemonics on or off" },
  XC: { syntax: "XC [OFF]", desc: "Extended opcodes (65C02 are always on)" },
  KBD: { syntax: "]var KBD value", desc: "Merlin prompted here; the value is used instead" },
  MX: { syntax: "MX %bits", desc: "65816 register widths (accepted, no effect)" },
  TR: { syntax: "TR ON|OFF", desc: "Truncation control (listing only)" },
  DAT: { syntax: "DAT", desc: "Date stamp (listing only)" },
  EXP: { syntax: "EXP ON|OFF", desc: "Macro expansion in the listing" },
  PAU: { syntax: "PAU", desc: "Pause the listing" },
  PAG: { syntax: "PAG", desc: "Page eject in the listing" },
  TTL: { syntax: "TTL text", desc: "Listing title" },
  SKP: { syntax: "SKP count", desc: "Skip lines in the listing" },
  USR: { syntax: "USR ...", desc: "User-defined directive (accepted, no effect)" },
  REL: { syntax: "REL", desc: "Relocatable code — needs a linker, unsupported" },
  ENT: { syntax: "ENT", desc: "Entry point — needs a linker, unsupported" },
  EXT: { syntax: "EXT", desc: "External reference — needs a linker, unsupported" },
  LNK: { syntax: "LNK filename", desc: "Link object file — unsupported" },
};
