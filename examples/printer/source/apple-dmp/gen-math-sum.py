#!/usr/bin/env python3
# Generator for MATH.SUM  -  Apple DMP custom-font math demo.
#
# Reproduces the TeX inline-vs-display summation gag on the Apple Dot
# Matrix Printer (1982, rebadged C.Itoh 8510):
...
#                         i=1
#
# Like the ImageWriter I, the DMP has NO super/subscript commands (ESC
# x/y/z arrived with the IW-II) and no half-height mode. The substitute
# is half-line REVERSE and FORWARD feeds (DMP User's Manual Part I,
# "Line Feed Direction" / "Line Feed Pitch"):
#     ESC T nn   line pitch nn/144"
#     ESC r/f    reverse / forward line feeding
...
# DOWNLOADED glyphs (mini i, =, 1) so the limits read script-sized.
# Custom chars (manual, "Custom Characters", pp.62-66): ESC + (16-col
# bank), ESC I..CTRL-D download (width code 0x40+n = top 8 wires, column
# bytes bit0 = top wire), ESC ' select / ESC $ off - the same C.Itoh
# wire format as both ImageWriters.
#
# CRITICAL - end-of-line guard: the DMP's factory DIP SW1-7 is CLOSED,
# making LF (and VT/FF) end-of-line characters that return the carriage
# to the left margin - which would fling the head away mid-hop. The DMP
# has no ESC l; its manual's spelling is software switch A-7:
#     ESC Z H R   end-of-line at CR only  (LF = pure paper feed)
#     ESC D H R   restore CR,LF,FF end-of-line (factory default)
# ('H' = switch A-7 plus unused A-4; 'R' touches only unused B bits.)
#
# Emits an Applesoft program that POKEs the raw byte stream straight to
# the parallel card (slot 1) data latch ($C090 = 49296, auto-strobe).
# That path is 8-bit transparent (no MSB mask, no CTRL-D interception).
...
# Mini script-sized subscript chars (6 tall) - the DMP cannot shrink its
...
REV = [ESC, 0x72]                          # ESC r : reverse line feeding
FWD = [ESC, 0x66]                          # ESC f : forward line feeding
LF  = [10]
...
# The ESC Z H R guard (see header) keeps LF from returning the carriage.
...
stream += EOL_CR_ONLY                      # LF = pure paper feed for the hops
...
# L4: i=1 centred UNDER the sigma (absolute ESC F - the C.Itoh backspace steps
...
# ------------------------------------------------------- emit Applesoft --------
prog = [
 "10  REM  SUM 1/I - APPLE DMP CUSTOM FONT",
 "20  REM  NO ESC X/Y/Z ON THE DMP: LIMITS STACKED WITH",
 "30  REM  ESC T + ESC R/F HALF-LINE FEEDS + ESC Z H R GUARD",
 "40  A = 49296: REM PARALLEL SLOT1 DATA LATCH ($C090)",
 "50  READ B: IF B < 0 THEN 80",
 "60  POKE A,B",
 "70  GOTO 50",
 "80  PRINT CHR$(4);\"SAVE /SCREEN.TO.PRINT/APPLE.DMP/MATH.SUM\"",
 "90  END",
]
ln, i, per = 100, 0, 16
while i < len(stream):
